...
class Container extends Component {
	 ...
	 */
	function insertImp(i:Int, com:Component, ?constraints:Dynamic=null):Void{
		if(i > getComponentCount()){
			//throw new RangeError("illegal component position when insert comp to container");
			throw ("illegal component position when insert comp to container");
		}
	...
	function checkChildRemoval(child:DisplayObject):Void{
		if(Std.is( child, Component)){
			...
			if(c.getParent() != null){
				...
				throw ("You should call remove method to remove a component child!");
			}
		}
	}
	...
	function removeAtImp(i:Int):Component{
		if(i < 0){
			return null;
		}
	...
	public function getComponent(index:Int):Component{
		if(index < 0 || index >= children.length){
			...
			throw ("Index out of container children bounds!!!");
		}
	...
	function getChildIndexWithComponentIndex(index:Int):Int{
		var count:Int = getComponentCount();
		if(index < 0 || index > count){
			//throw new RangeError("Out of index counting bounds, it should be >=0 and <= component count!");
			throw ("Out of index counting bounds, it should be >=0 and <= component count!");
		}
		if(index == count){
	...
	function getComponentIndexWithChildIndex(index:Int):Int{
		var count:Int = numChildren;
		if(index < 0 || index > count){
			//throw new RangeError("Out of index counting bounds, it should be >=0 and <= numChildren!");
			throw ("Out of index counting bounds, it should be >=0 and <= numChildren!");
		}
		if(index == count){
	 ...
	 */
	override function countMinimumSize():IntDimension{
	 ...
	 * call the ui, if ui return null, ehn call layout to count.
	 */
	override function countMaximumSize():IntDimension{
		var size:IntDimension = null;
		if(ui != null){
