...
class FocusManager {
	...
	static var defaultTraversalEnabled:Bool = true;
	
	var oldFocusOwner:Component;
	var focusOwner:Component;
	var popups:ArrayList;
	var activeWindow:JWindow;
	var traversalEnabled:Bool ;
	var traversalDefault:Bool ;
		
	var stage:Stage;
	var focusRect:Sprite;
	var inited:Bool;
	var defaultPolicy:FocusTraversalPolicy;
	var traversing:Bool;
	...
	var focusPaintedComponent:Component;
	...
	function addListenerToFocusPaintedComponent():Void{
	...
	function removeistenerToFocusPaintedComponent():Void{
		if(focusPaintedComponent != null){
	...
	function __focusPaintedComRemoved(e:Event):Void{
		focusRect.graphics.clear();
		removeistenerToFocusPaintedComponent();
	}
	
	function __focusPaintedComMoved(e:MovedEvent):Void{
	...
	function __focusPaintedComResized(e:ResizedEvent):Void{
		if(focusRect.visible){
	...
	function __onMouseFocusChange(e:FocusEvent):Void{
		...
		if(focusOwner != null){
			...
			if(AsWingManager.isPreventNullFocus() 
				...
				|| (Std.is( tar, Component) && !cast(tar, Component).isFocusable())){
				e.preventDefault();
			}
		}
	}
	
	function __onKeyFocusChange(e:FocusEvent):Void{
