...
class JToolTip extends Container {
	...
	//the time waiting after to view tool tip when roll over a component
	static var WAIT_TIME:Int = 600;
	//when there is one tooltip is just shown, next will shown fast as this time
	static var FAST_OCCUR_TIME:Int = 50;
	
	static var last_tip_dropped_time:Int = 0;
	
	static var defaultRoot:DisplayObjectContainer = null;
	
	var containerRoot:DisplayObjectContainer;
	
	var tipText:String;
	var comp:InteractiveObject;
	var offsets:IntPoint;
	var offsetsRelatedToMouse:Bool;
	
	var timer:Timer;
	var waitThenPopupEnabled:Bool;
	...
	static function getDefaultToolTipContainerRoot():DisplayObjectContainer{
		if(defaultRoot == null){
	...
	function getToolTipContainerRoot():DisplayObjectContainer{
		if(containerRoot == null){
			var cr:DisplayObjectContainer;
			if(getTargetComponent() != null){
				cr = cast( getTargetComponent().root, DisplayObjectContainer);
			}
			if(cr == null){
	...
	function __compRollOver(source:InteractiveObject):Void{
		if(source == comp && isWaitThenPopupEnabled()){
			startWaitToPopup();
		}
	}
	
	function __compRollOut(source:InteractiveObject):Void{
		if(source == comp && isWaitThenPopupEnabled()){
			disposeToolTip();
		}
	}
		
	function __onMouseMoved(e:Event):Void{
		if(timer.isRunning()){
			timer.restart();
		}
	}
	
	function __timeOnAction(e:Event):Void{
		timer.stop();
	...
	function listenOwner(comp:InteractiveObject, ?useWeakReference:Bool = false):Void{
	...
	function unlistenOwner(comp:InteractiveObject):Void{
	...
	function ____compRollOver(e:Event):Void{
		var source:InteractiveObject = cast( e.currentTarget, InteractiveObject);
		__compRollOver(source);
	}
	
	function ____compRollOut(e:Event):Void{
		var source:InteractiveObject = cast( e.currentTarget, InteractiveObject);
