...
class VectorListModel extends AbstractListModel, implements MutableListModel, implements List {
	...
	public function appendAll(arr:Array<Dynamic>, ?index:Int=-1):Void{
		...
		if(index == 0){
			...
			var left:Array<Dynamic> = _elements.slice(0, index);
            var right:Array<Dynamic> = _elements.slice(index, _elements.length);
			_elements = left.concat(arr);
	...
	public function sort(compare:Dynamic, options:Int):Array<Dynamic>{
		//var returned:Array<Dynamic> = _elements.sort(compare, options);
		_elements.sort(compare);
		var returned:Array<Dynamic> = _elements;
		fireContentsChanged(this, 0, _elements.length-1, []);
		return returned;
	}
	
	public function sortOn(key:Dynamic, options:Int):Array<Dynamic>{
		var returned:Array<Dynamic> = untyped _elements.sortOn(key, options);
		fireContentsChanged(this, 0, _elements.length-1, []);
		return returned;
	}
