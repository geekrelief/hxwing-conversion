...
class JWindow extends JPopup {
	...
	var focusWhenDeactive:Component;
	var lootActiveFrom:JWindow;
	...
	public override function setLayout(layout:LayoutManager):Void{
		if(Std.is( layout, WindowLayout)){
			...
			throw ("Can not set a non-WindowLayout Layout to JWindow");
		}
	}
	 ...
	 */
	public override function paintImmediately():Void{
		if(isVisible()){
	...
	function setContentPaneImp(cp:Container):Void{
		if(contentPane != null){
	...
	override function disposeProcess(st:Stage):Void{
		lostActiveAction(st);
	}	
	...
	*/
	...
	function lostActiveAction(st:Stage):Void{
		if(isActive()){
			deactive(st);
	...
	function getLootActiveFrom():JWindow{
		return lootActiveFrom;
	}
	function setLootActiveFrom(activeOwner:JWindow):Void{
	...
	function __lootActiveFromHide(e:PopupEvent):Void{
		if(lootActiveFrom != null){
			setLootActiveFrom(lootActiveFrom.lootActiveFrom);
		}
	}
	
	function active(?programmatic:Bool=true):Void{
	...
	function deactive(st:Stage, ?programmatic:Bool=true):Void{
		actived = false;
	...
	function __activeWhenPress(e:MouseEvent):Void{
	...
	function __activeWhenPressWindowSelf(e:MouseEvent):Void{
		if(e.target == this){
			__activeWhenPress(e);
		}
	}
