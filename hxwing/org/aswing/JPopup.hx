...
class JPopup extends JRootPane {
	...
	var ground_mc:Sprite;
	var modalMC:Sprite;
	var owner:Dynamic;
	var modal:Bool;
	
	var ownedEquipedPopups:ArrayList;
	var lastLAF:Dynamic;	
	...
	public function new(?owner:Dynamic=null, ?modal:Bool=false){
		...
		if(Std.is( owner, JPopup) || Std.is( owner, DisplayObjectContainer)){
			...
			throw (this + " JPopup's owner is not a DisplayObjectContainer or JPopup, owner is : " + owner);
		}
		this.modal = modal;
		setName("JPopup");
	...
	function __popupOpennedAddToList(e:Event):Void{
	...
	function __popupOfffromDisplayList(e:Event):Void{
		var fm:FocusManager = FocusManager.getManager(stage);
	 ...
	 */
	function disposeProcess(st:Stage):Void{
	}	
	...
	var lastDragPos:IntPoint;
	public override function startDrag(?lockCenter:Bool=false, ?bounds:Rectangle=null):Void{
		if(stage != null){
	...
	function __dragMoving(e:MouseEvent):Void{
		bounds.x = d_x;
		bounds.y = d_y;
	...
	public static function getOwnedPopupsWithOwner(owner:DisplayObjectContainer):Array<Dynamic>{
		...
		if(fm != null){
			var ws:Array<Dynamic> = new Array();
	...
	function addOwnedEquipedPopup(pop:JPopup):Void{
		ownedEquipedPopups.append(pop);
	}
	
	function removeOwnedEquipedPopup(pop:JPopup):Void{
		ownedEquipedPopups.remove(pop);
	}
	
	//--------------------------------------------------------
		
	function __resetModelMCWhenStageResized(e:Event):Void{
		if(isVisible()){
			resetModalMC();
		}
	}
	
	function equipPopupContents():Void{
		if(Std.is( owner, JPopup)){
			...
			throw (this + " JPopup's owner is not a mc or JPopup, owner is : " + owner);
		}
