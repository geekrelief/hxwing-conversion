...
class ContainerOrderFocusTraversalPolicy implements FocusTraversalPolicy {
	...
	function getComponentAfterImp(c:Component, ?deepIn:Bool=true):Component{
		if(c == null){
			return null;
		}
	...
	function getComponentBeforeImp(c:Component):Component{
		if(c == null){
			return null;
		}
		var container:Container = c.getParent();
	 ...
	 */
	function getFirstComponent(container:Container):Component{
		if(container == null){
			return null;
		}
		var index:Int = -1;
	 ...
	 */
	function getLastComponent(c:Component):Component{
	...
	function isLeaf(c:Component):Bool{
	...
	function accept(c:Component):Bool{
		return c != null && c.isShowing() && c.isFocusable() && c.isEnabled();
	}
	
	function getFocusableComponent(c:Component):Component{
