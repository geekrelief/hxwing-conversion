...
class JTextArea extends JTextComponent, implements Viewportable {
	...
	static var defaultMaxChars:Int = 0;
 	
	var columns:Int;
	var rows:Int;
	
	var viewPos:IntPoint;
	var viewportSizeTesting:Bool;
	var lastMaxScrollV:Int;
	var lastMaxScrollH:Int;
	...
	override function isAutoSize():Bool{
	...
	override function size():Void{
		super.size();
		applyBoundsToText(getPaintBounds());
	}

	
	function __onTextAreaTextChange(e:Event):Void{
	...
	function __onTextAreaTextScroll(e:Event):Void{
    	if(viewportSizeTesting){
    		return;
    	}
