...
class JProgressBar extends Component, implements Orientable {
	...
	var indeterminatePaintTimer:Timer;
	var indeterminateDelaySet:Bool;
	 ...
	 */
	public function new(?orient:Int, ?min:Int=0, ?max:Int=100) {
		super();
		setName("ProgressBar");

        if(orient == null){
            orient = AsWingConstants.HORIZONTAL;
        }
	...
	function addListenerToModel():Void{
		model.addStateListener(__onModelStateChanged);		
	}
	
	function __progressAddedToStage(e:Event):Void{
		__validateIndeterminateIntervalIfNecessary();
	}
	
	function __progressRemovedFromStage(e:Event):Void{
		__validateIndeterminateIntervalIfNecessary();
	}
	...
	function __indeterminateInterval(e:AWEvent):Void{
		repaint();
	}
	
	function __validateIndeterminateIntervalIfNecessary():Void{
