 ...
 * @see VectorListModel
 */
class JList extends Container, implements LayoutManager, implements Viewportable, implements org.aswing.event.ListDataListener {
	
    public static var INT_MAX:Int = (2<<30)-1;	
 	/**
 	 * The default unit/block increment, it means auto count a value.
 	 */
 	public static var AUTO_INCREMENT:Int = 2<<30;
	...
	var viewHeight:Int;
	var viewWidth:Int;
	var maxWidthCell:ListCell;
	var cellPrefferSizes:HashMap; //use for catche sizes when not all cells same height
	var comToCellMap:HashMap; 
	...
	var firstVisibleIndexOffset:Int ;
	var lastVisibleIndexOffset:Int ;
	
	var autoDragAndDropType:Int;
	...
	public override function setLayout(layout:LayoutManager):Void{
		...
		throw ("Can not set layout to JList, its layout is itself!");
	}	
	...
	public function getSelectedIndices():Array<Dynamic>{
		var indices:Array<Dynamic> = new Array();
		...
		if(factory.isAllCellHasSameHeight() || factory.isShareCells()){
			...
			for(i in 0...num){
				...
				if(s == null){
					s = cell.getCellComponent().getPreferredSize();
                    #if debug
					trace("Warning : cell size not cached index = " + i + ", value = " + cell.getCellValue());
                    #end
				}
				y += s.height;
			}
	...
	public function scrollToBottomLeft():Void{
		setViewPosition(new IntPoint(0, INT_MAX));
	}
	/**
	...
	public function scrollToBottomRight():Void{
		setViewPosition(new IntPoint(INT_MAX, INT_MAX));
	}
	/**
	...
	public function scrollToTopRight():Void{
		setViewPosition(new IntPoint(INT_MAX, 0));
	}	
	
	/**
     * Enables the list so that items can be selected.
     */
	public override function setEnabled(b:Bool):Void{
		super.setEnabled(b);
		mouseChildren = b;
	}
	...
	function isAutoDragAndDropAllown():Bool{
	...
	function addCellToContainer(cell:ListCell):Void{
	...
	function removeCellFromeContainer(cell:ListCell):Void{
	...
	function checkCreateCellsWhenShareCells():Void{
		createCellsWhenShareCells();
	}
	
	function createCellsWhenShareCells():Void{
	...
	function createCellsWhenNotShareCells():Void{
		var factory:ListCellFactory = getCellFactory();
    ...
    function layoutWhenShareCells():Void{
    ...
    function layoutWhenNotShareCellsAndSameHeight():Void{
    	var factory:ListCellFactory = getCellFactory();
		var m:ListModel = getModel();
		var ir:IntRectangle = getInsets().getInsideBounds(getSize().getBounds());
		...
		while (i<=endIndex){
			...
			var finalWidth:Int = isTracksWidth() ? ir.width : Std.int(Math.max(cellWidth, s.width));
    ...
    function getCachedCellPreferSize(cell:ListCell):IntDimension{
    	return cast(cellPrefferSizes.get(cell.getCellComponent()), IntDimension);
    }
    
    function layoutWhenNotShareCellsAndNotSameHeight():Void{
		var m:ListModel = getModel();
		var ir:IntRectangle = getInsets().getInsideBounds(getSize().getBounds());
    	var cellWidth:Int = Std.int(Math.max(ir.width, viewWidth));
    	ir.x = ir.y = 0; //this is because the cells is in cellPane, not in JList
    ...
    function __selectionListener(e:SelectionEvent):Void{
    ...
    function addHandlersToCell(cellCom:Component):Void{
    ...
    function removeHandlersFromCell(cellCom:Component):Void{
	...
	function createItemEventObj(cellCom:Dynamic, type:String, e:MouseEvent):ListItemEvent{
	...
	function getItemIndexByCellComponent(item:Component):Int{
	...
	function getCellByCellComponent(item:Component):ListCell{
     ...
     * Event Listener For All Items
     */
	function __onItemMouseDown(e:MouseEvent):Void{
     ...
     * Event Listener For All Items
     */	
	function __onItemClick(e:MouseEvent):Void{
     ...
     * Event Listener For All Items
     */	
	function __onItemReleaseOutSide(e:ReleaseEvent):Void{
     ...
     * Event Listener For All Items
     */	
	function __onItemRollOver(e:MouseEvent):Void{
     ...
     * Event Listener For All Items
     */	
	function __onItemRollOut(e:MouseEvent):Void{
     ...
     * Event Listener For All Items
     */	
	function __onItemDoubleClick(e:MouseEvent):Void{
