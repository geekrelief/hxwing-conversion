...
class JTextComponent extends Component, implements EditableComponent {
	...
	var rowHeight:Int;
	var widthMargin:Int;
	var heightMargin:Int;
	var columnRowCounted:Bool;	
	...
	function __onTextComponentTextInput(e:TextEvent):Void{
	 ...
	 */
	function invalidateColumnRowSize():Void{
		columnRowCounted = false;
	}	
	 ...
	 * @see #getRowHeight()
	 */	
	function getHeightMargin():Int{
		if(!columnRowCounted) countColumnRowSize();
		return heightMargin;
	}
	
	function getTextFieldAutoSizedSize(?forceWidth:Int=0, ?forceHeight:Int=0):IntDimension{
		var tf:TextField = getTextField();
		var oldSize:IntDimension = new IntDimension(Std.int(tf.width), Std.int(tf.height));
		...
		var size:IntDimension = new IntDimension(Std.int(tf.width), Std.int(tf.height));
		tf.autoSize = old;
	...
	function countColumnRowSize():Void{
    ...
    function applyBoundsToText(b:IntRectangle):Void{
