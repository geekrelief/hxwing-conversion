...
class Graphics2D  {
	...
	function setTarget(target:Graphics):Void{
		this.target = target;
	}
	...
	function startPen(p:IPen):Void{
		p.setTo(target);
	}
	
	function endPen():Void{
		target.lineStyle();
		target.moveTo(0, 0); //avoid a drawing error
	}
	
	function startBrush(b:IBrush):Void{
		brush = b;
		b.beginFill(target);
	}
	 ...
	 */
	public function drawRoundRect(pen:IPen, x:Float, y:Float, width:Float, height:Float, radius:Float, ?trR:Int=-1, ?blR:Int=-1, ?brR:Int=-1):Void{
		startPen(pen);
	 ...
	 */	
	public function fillRoundRect(brush:IBrush,x:Int,y:Int,width:Int,height:Int, radius:Int, ?topRightRadius:Int=-1, ?bottomLeftRadius:Int=-1, ?bottomRightRadius:Int=-1):Void{
		startBrush(brush);
	 ...
	 */	
	public function fillRoundRectRingWithThickness(brush:IBrush, x:Int, y:Int, width:Int, height:Int, radius:Int, thickness:Int, ?innerRadius:Int=-1):Void{
		startBrush(brush);
...
//		target.curveTo(cx - r, cy - r, cx, cy - r);
	 ...
	 * @see #fillRoundRect()
	 */
	public function roundRect(x:Int,y:Int,width:Int,height:Int, radius:Int, ?topRightRadius:Int=-1, ?bottomLeftRadius:Int=-1, ?bottomRightRadius:Int=-1):Void{
	...
	public function wedge(radius:Float, x:Float, y:Float, angle:Float):Void {
		...
		var nSeg:Int = Math.floor(angle/30);
		var pSeg:Int = angle-nSeg*30;
		var a:Float = 0.268;
		var endx:Float;
		var endy:Float;
		var ax:Float;
