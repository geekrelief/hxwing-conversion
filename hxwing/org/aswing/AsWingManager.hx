...
class AsWingManager {
    ...
    static var INITIAL_STAGE_WIDTH:Int;
    static var INITIAL_STAGE_HEIGHT:Int;
    static var timer:Timer;
    static var nextFrameCalls:Array<Dynamic> = new Array();
    static var preventNullFocus:Bool = true;
	...
	public static function callLater(func:Dynamic, ?time:Int=40):Void{
	...
	static function __update(e:TimerEvent):Void{
		e.updateAfterEvent();
	}
	
	static function __enterFrame(e:Event):Void{
		var calls:Array<Dynamic> = nextFrameCalls;
		nextFrameCalls = new Array();
		for(i in 0...calls.length){
			var func:Dynamic = calls[i];
			func();
		}
	}
}
