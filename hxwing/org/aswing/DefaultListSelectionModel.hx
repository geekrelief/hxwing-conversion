...
class DefaultListSelectionModel extends EventDispatcher, implements ListSelectionModel {
	...
	function updateLeadAnchorIndices(anchor:Int, lead:Int):Void {
		anchorIndex = anchor;
		leadIndex   = lead;
	}
	...
	function clearSelectionImp():Void{
		value = [];
	}
	...
	public function insertIndexInterval(index:Int, length:Int, before:Bool, ?programmatic:Bool=true):Void{
		...
		if(needInstertArray){
			if(insMinIndex == 0){
				...
				var right:Array<Dynamic> = value.splice(insMinIndex);
	...
	public function addListSelectionListener(listener:Dynamic):Void{
		addEventListener(SelectionEvent.LIST_SELECTION_CHANGED, listener);
	}
	
	public function removeListSelectionListener(listener:Dynamic):Void{
		removeEventListener(SelectionEvent.LIST_SELECTION_CHANGED, listener);
	}
	
	function fireListSelectionEvent(firstIndex:Int, lastIndex:Int, programmatic:Bool):Void{
