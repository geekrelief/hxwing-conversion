...
class JPopupMenu extends Container, implements MenuElement {
	...
	static var popupMenuMouseDownListening:Bool = false;
	static var showingMenuPopups:HashSet = new HashSet();
	
	var selectionModel:SingleSelectionModel;
	
	var invoker:Component;
	var popup:JPopup;
	var menuInUse:Bool;
    ...
    public override function setUI(newUI:ComponentUI):Void{
    	if(Std.is( newUI, MenuElementUI)){
    		...
    		throw ("JPopupMenu just accept MenuElementUI instance!!!");
    	}
    }
	 ...
	 *		  hide it
	 */
	public override function setVisible(b:Bool):Bool {
		if (b == isVisible())
			return b;
...
//		invalidate();
	 ...
	 */	
	function isPopupMenu():Bool{
		return (!(Std.is( invoker, JMenu)));
	}
	
	function adjustPopupLocationToFitScreen(gp:IntPoint):IntPoint{
	...
	function __popMenuChildAdd(e:ContainerEvent) : Void {
	...
	function __popMenuChildRemove(e:ContainerEvent) : Void {
		var child:Component = e.getChild();
		if(Std.is( child, MenuElement)){
	...
	static function __popupMenuMouseDown(e:Event):Void{
	...
	function __popupShown(e:PopupEvent) : Void {
	...
	function __popupClosed(e:PopupEvent) : Void {
		var source:Dynamic = e.target;
