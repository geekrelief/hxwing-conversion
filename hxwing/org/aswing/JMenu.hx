...
class JMenu extends JMenuItem, implements MenuElement {
	...
	function ensurePopupMenuCreated() : Void {
        if (popupMenu == null) {
	...
	function getPopupMenuOrigin() : IntPoint {
		var p:IntPoint;
		if(Std.is( getParent(), JPopupMenu)){
	...
	function getUIPropertyNumber(name:String):Int{
		var n:Int = getUI().getInt(name);
		return n;
	}
	
	function __menuDestroied(e:Event):Void{
    ...
    public override function setInUse(b:Bool):Void{
    	if(menuInUse != b){
	    	menuInUse = b;
	    	if(b){
	    		ensurePopupMenuCreated();
	...
	public override function menuSelectionChanged(isIncluded : Bool) : Void {
		setSelected(isIncluded);
	}

	public override function getSubElements() : Array<Dynamic> {
        if(popupMenu == null){
            return [];
        }else{
            return [popupMenu];
