 ...
 * @author iiley
 */
class JAttachPane extends AssetPane {
	 ...
	 * @see #setPath()
	 */
	public function new(?assetClassName:String=null, ?prefferSizeStrategy:Int=1, ?applicationDomain:ApplicationDomain=null) {
	...
	function createAsset():DisplayObject{
		if(className == null){
			return null;
		}
		var classReference:Class<Dynamic>;
		if (applicationDomain == null){
			classReference = Type.resolveClass(className);
		}else{
			classReference = cast( applicationDomain.getDefinition(className), Class<Dynamic>);
		}
		if(classReference == null){
			return null;
		}
		var attachMC:DisplayObject = cast( Type.createInstance(classReference, []), DisplayObject);
		if(attachMC == null){
			return null;
		}
		setAssetOriginalSize(new IntDimension(Std.int(attachMC.width), Std.int(attachMC.height)));
