...
class AbstractButton extends Component {
    ...
    var textFilters:Array<Dynamic> ;
	
	public function new(?text:String="", ?icon:Icon=null){
		
		shiftOffset = 0;
		shiftOffsetSet =false;
		textFilters = null;
		super();
    ...
    */
	...
	var rootPane:JRootPane;
	function __addedToStage(e:Event):Void{
	...
	function __rollOverListener(e:MouseEvent):Void{
	...
	function __rollOutListener(e:MouseEvent):Void{
		var m:ButtonModel = getModel();
		if(isRollOverEnabled()) {
			if(!m.isPressed()){
				m.setRollOver(false);
	...
	function __mouseReleaseListener(e:Event):Void{
	...
	function __modelActionListener(e:AWEvent):Void{
		dispatchEvent(new AWEvent(AWEvent.ACT));
	}
	
	function __modelStateListener(e:AWEvent):Void{
		dispatchEvent(new InteractiveEvent(InteractiveEvent.STATE_CHANGED));
	}
	
	function __modelSelectionListener(e:AWEvent):Void{
