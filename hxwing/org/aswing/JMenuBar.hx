...
class JMenuBar extends Container, implements MenuElement {
    ...
    public override function setUI(newUI:ComponentUI):Void{
    	if(Std.is( newUI, MenuElementUI)){
    		...
    		throw ("JMenuBar just accept MenuElementUI instance!!!");
    	}
    }
	...
	function __menuBarDestroied(e:Event):Void{
		setInUse(false);
	}
	
	function __menuBarCreated(e:Event):Void{
		setInUse(true);
	}
	
	function __menuBarChildAdd(e:ContainerEvent) : Void {
		if(Std.is( e.getChild(), MenuElement)){
			cast(e.getChild(), MenuElement).setInUse(isInUse());
		}
	}

	function __menuBarChildRemove(e:ContainerEvent) : Void {
		if(Std.is( e.getChild(), MenuElement)){
