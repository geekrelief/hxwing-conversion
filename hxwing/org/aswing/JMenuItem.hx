...
class JMenuItem extends AbstractButton, implements MenuElement {
    ...
    public override function setUI(newUI:ComponentUI):Void{
    	if(Std.is( newUI, MenuElementUI)){
    		...
    		throw ("JMenuItem just accept MenuElementUI instance!!!");
    	}
    }
	 ...
	 */
	function initFocusability():Void {
		setFocusable(false);
	}
	...
	function inUseChanged():Void{
		...
		if(acc != null){
			...
			if(rOwner == null){
                ...
                */
				throw ("The menu item has accelerator, " + 
					    "it or it's popupMenu must be in a JRootPane(or it's subclass).");
				return;
			}
	...
	function __acceleratorAction():Void{
		doClick();
	}
