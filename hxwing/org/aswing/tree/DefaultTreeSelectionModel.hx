 ...
 * @author iiley
 */
class DefaultTreeSelectionModel extends EventDispatcher, implements org.aswing.tree.TreeSelectionModel {
    ...
    /** Paths that are currently selected.  Will be null if nothing is
      * currently selected. */
    var selection:Array<Dynamic>; //TreePath[]
    ...
    var listSelectionModel:DefaultListSelectionModel;
     ...
     */
    var uniquePaths:TreePathMap;
    var lastPaths:TreePathMap;
    var tempPaths:Array<Dynamic>; //TreePath[]
    ...
    function firePropertyChange(name:String, oldValue:Dynamic, newValue:Dynamic):Void{
    ...
    public function addSelectionPaths(paths:Array<Dynamic>, ?programmatic:Bool=true):Void {
	    ...
	    if(selectionMode == SINGLE_TREE_SELECTION) {
			...
			if(validCount > 0) {
			    var newSelection:Array<Dynamic> = new Array();
    ...
    public function removeSelectionPaths(paths:Array<Dynamic>, ?programmatic:Bool=true):Void {
		if (paths != null && selection != null && paths.length > 0) {
		    if(!canPathsBeRemoved(paths)) {
				...
				var removeCounter:Int = paths.length - 1;
    ...
    public function clearSelection(?programmatic:Bool=true):Void {
		if(selection != null) {
		    ...
		    var newness:Array<Dynamic> = new Array(); //boolean[]
     ...
     */
    function fireValueChanged(e:TreeSelectionEvent):Void {
    	dispatchEvent(e);
    }
		    ...
		    if (rows != null) {
				...
				if (invisCount > 0) {
				    if (invisCount == rows.length) {
						...
						var tempRows:Array<Dynamic> = new Array();
						counter = rows.length - 1;
     ...
     */
    public function addPropertyChangeListener(listener:Dynamic, ?priority:Int=0, ?useWeakReference:Bool=false):Void{
	 ...
	 */
	public function removePropertyChangeListener(listener:Dynamic):Void{
    ...
    /**
      ...
      */
    public function addTreeSelectionListener(listener:Dynamic, ?priority:Int=0, ?useWeakReference:Bool=false):Void{
	 ...
	 * @param listener the listener to be removed.
	 */    
    public function removeTreeSelectionListener(listener:Dynamic):Void{
     ...
     */
    function insureRowContinuity():Void {
                ...
                while (counter <= maxCounter) {
				    if(!lModel.isSelectedIndex(counter)) {
						if(counter == min) {
						    ...
						    var newSel:Array<Dynamic> = new Array();
     ...
     */
    function arePathsContiguous(paths:Array<Dynamic>):Bool {
		if(rowMapper == null || paths.length < 2){
		    return true;
		}else {
		    var bitSet:Array<Dynamic> = new Array();
     ...
     */
    function canPathsBeAdded(paths:Array<Dynamic>):Bool {
		if(paths == null || paths.length == 0 || rowMapper == null ||
		   selection == null || selectionMode == DISCONTIGUOUS_TREE_SELECTION){
		    ...
		    var tempPath:Array<Dynamic> = new Array();
     ...
     */
    function canPathsBeRemoved(paths:Array<Dynamic>):Bool {
    ...
    /**
      ...
      */
    function notifyPathChange(changedPaths:ArrayList, oldLeadSelection:TreePath, programmatic:Bool):Void {
		var cPathCount:Int = changedPaths.size();
		var newness:Array<Dynamic> = new Array(); //boolean[]
		var paths:Array<Dynamic> = new Array(); //TreePath[]
    ...
    function updateLeadIndex():Void {
		if(leadPath != null) {
		    if(selection == null) {
				...
				leadRow = leadIndex = -1;
				var counter:Int = selection.length - 1;
     ...
     */
    function insureUniqueness():Void {
    }
