...
class FixedHeightLayoutCache extends AbstractLayoutCache {
	 ...
	 */
	var boundsBuffer:IntRectangle;

	/**
	 * Maps from TreePath to a FHTreeStateNode.
	 */
	var treePathMapping:TreePathMap;
	 ...
	 * @see #rootVisible
	 */
	public override function setRootVisible(rootVisible:Bool):Void {
	...
	/**
	  ...
	  */
	public override function isExpanded(path:TreePath):Bool {
		if(path != null) {
	 ...
	 */
	public override function getVisiblePathsFrom(path:TreePath, totalCount:Int):Array<Dynamic> {
		if(path == null)
			return null;
	...
	function nextVisibleFHTreeStateNode(info:EnumerationInfo):TreePath{		
	...
	function hasMoreVisibleFHTreeStateNode(info:EnumerationInfo):Bool{
		return info.enumCount > 0 && info.parent != null;
	}
	
	function updateNextObject(info:EnumerationInfo):Void{
		if(!updateNextIndex(info)) {
			findNextValidParent(info);
		}
	}
	function updateNextIndex(info:EnumerationInfo):Bool{
	...
	function findNextValidParent(info:EnumerationInfo):Bool{
		if(info.parent == root) {
			// mark as invalid!
			info.parent = null;
			return false;
	...
	function getVisibleFHTreeStateNodes(parent:FHTreeStateNode, startIndex:Int, totalCount:Int):Array<Dynamic>{
	 ...
	 */
	public override function setExpandedState(path:TreePath, isExpanded:Bool):Void {
		if(isExpanded){
	 ...
	 */
	public override function getExpandedState(path:TreePath):Bool {
		var node:FHTreeStateNode = getNodeForPath(path, true, false);
	...
	function visibleNodesChanged():Void {
	}
	 ...
	 */
	function getBounds2(parent:FHTreeStateNode, childIndex:Int, placeIn:IntRectangle):IntRectangle {
		var expanded:Bool;
		var level:Int;
		var row:Int;
		var value:Dynamic;
	 ...
	 */
	function getMapping(path:TreePath):FHTreeStateNode {
	 ...
	 */
	function rebuild(clearSelection:Bool):Void {
	 ...
	 */
	function ensurePathIsExpanded(aPath:TreePath, expandLast:Bool):Bool {
		if(aPath != null) {
	 ...
	 */
	function getNodeForPath(path:TreePath, onlyIfVisible:Bool, shouldCreate:Bool):FHTreeStateNode {
		if(path != null) {
