...
class TableModelEvent extends ModelEvent {
    ...
    public static var HEADER_ROW:Int = -1;

    /** Specifies all columns in a row or rows. */
    public static var ALL_COLUMNS:Int = -1;
     ...
     * </ul>
     */
    public function new(source:TableModel, ?firstRow:Int=-2, ?lastRow:Int=-2, ?column:Int, ?type:String) {
    	...
    	super(source);
    	if(firstRow == -2){
	        // Use INT_MAX instead of getRowCount() in case rows were deleted. 
			init(0, INT_MAX, column, type);
    	}else if(lastRow == -2){
	...
	function init(firstRow:Int, lastRow:Int, column:Int, type:String):Void{
