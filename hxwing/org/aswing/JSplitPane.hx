...
class JSplitPane extends Container, implements Orientable {
     ...
     */
    public static var VERTICAL_SPLIT:Int = AsWingConstants.VERTICAL;
     ...
     * other.
     */
    public static var HORIZONTAL_SPLIT:Int = AsWingConstants.HORIZONTAL;
     ...
     *        	vertically-split pane. Default is null.
     */
	public function new(?orientation:Int, ?continuousLayout:Bool=false, ?leftComponent:Component=null, ?rightComponent:Component=null) {
		super();
        if(orientation == null){
     ...
     */
    function setLastDividerLocation(newLastLocation:Int):Void {
    ...
    public function setDividerLocation(location:Int, ?programmatic:Bool=false):Void {
		...
		if(oldValue != location){
			...
			if(oldValue >= 0 && oldValue != INT_MAX){
