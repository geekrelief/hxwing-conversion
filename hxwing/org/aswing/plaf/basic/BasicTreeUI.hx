 ...
 * @private
 */
class BasicTreeUI extends BaseComponentUI, implements org.aswing.plaf.TreeUI, implements org.aswing.tree.NodeDimensions, implements org.aswing.event.TreeModelListener {
	
	
	
	static var EMPTY_INSETS:Insets;

	/** Object responsible for handling sizing and expanded issues. */
	var treeState:AbstractLayoutCache;	
	
	var rendererPane:CellPane;	
	/** Total distance that will be indented.  The sum of leftChildIndent
	  * and rightChildIndent. 
	  */
	var totalChildIndent:Int;
	...
	var ignoreLAChange:Bool;
	
	var tree:JTree;
	var treeModel:TreeModel;
	var editor:TreeCellEditor;
	var selectionModel:TreeSelectionModel;
	var cells:ArrayList;
	var validCachedViewSize:Bool;
	var viewSize:IntDimension;
	var lastViewPosition:IntPoint;
	
	var expandControl:ExpandControl;
	...
	function setModel(tm:TreeModel):Void{
		cancelEditing(tree);
		if(treeModel != null){
	...
	function setSelectionModel(sm:TreeSelectionModel):Void{
	 ...
	 */
	function setRootVisible(newValue:Bool):Void {
		cancelEditing(tree);
		updateDepthOffset();
		if(treeState != null) {
	...
	function setCellEditor(editor:TreeCellEditor):Void{
		cancelEditing(tree);
		this.editor = editor;
	}
	 ...
	 */
	function setEditable(newValue:Bool):Void {
		cancelEditing(tree);
		if(newValue){
			editor = tree.getCellEditor();
		}else{
	...
	function repaintPath(path:TreePath):Void {
	}
	
	function cellFactoryChanged():Void{
	 ...
	 */
	function updateLayoutCacheExpandedNodes():Void {
	 ...
	 */
	function isLocationInExpandControl(path:TreePath, mouseX:Int, mouseY:Int):Bool {
	 ...
	 */
	function toggleExpandState(path:TreePath):Void {
	...
	/**
	  ...
	  * visible.
	  */
	function ensureRowsAreVisible(beginRow:Int, endRow:Int):Void {
	 ...
	 * toggleExpandState.
	 */
	function handleExpandControlClick(path:TreePath, mouseX:Int, mouseY:Int):Void {
		toggleExpandState(path);
	}	
	
	function selectPathForEvent(path:TreePath, e:MouseEvent):Void{
	...
	function doSelectPathForEvent(e:MouseEvent):Void{
	...
	//------------------------------handlers------------------------
	function __selectionModelPropertyChanged(e:PropertyChangeEvent):Void{
		selectionModel.resetRowSelection();
	}
	function __selectionChanged(event:TreeSelectionEvent):Void{
		// Stop editing
		stopEditing(tree);
		...
		if(tree.isExpandsSelectedPaths() && selectionModel != null) {
			...
			if(paths != null) {
				var counter:Int = paths.length - 1;
				while (counter >= 0) {
	...
	function __onClicked(e:ClickCountEvent):Void{
	...
	function __onPressed(e:MouseEvent):Void{
		var p:IntPoint = rendererPane.getMousePosition();
		p.y += tree.getViewPosition().y;
		var path:TreePath = getClosestPathForLocation(tree, p.x, p.y);
		if(path != null){
	...
	var doSelectWhenRelease:Bool;
	var pressedPath:TreePath;
		
	function __onReleased(e:MouseEvent):Void{
	...
	function isControlKey(code:UInt):Bool{
	...
	function positRendererPaneX(viewX:Int):Void{
	 ...
	 */
	function updateDepthOffset():Void {
	...
	var rendererShape:Shape;
	function createRendererPaneGraphics():Graphics2D{
	...
	function paintExpandControl(g:Graphics2D, bounds:IntRectangle, path:TreePath,
	...
	function checkCreateCells():Void{
		var ih:Int = tree.getRowHeight();
	 ...
	 */
	function getRowX(row:Int, depth:Int):Int {
	 ...
	 * to the tree state.
	 */
	function updateExpandedDescendants(path:TreePath):Void {
		//completeEditing();
		if(treeState != null) {
	...
	//**********************************************************************
	var currentCellRenderer:TreeCell;
	...
	function updateCachedViewSize():Void{
		if(treeState != null) {
