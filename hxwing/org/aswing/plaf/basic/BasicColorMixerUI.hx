...
class BasicColorMixerUI extends BaseComponentUI {
	...
	var colorMixer:JColorMixer;
	var mixerPanel:JPanel;
	var HSMC:AWSprite;
	var HSPosMC:AWSprite;
	var LMC:AWSprite;
	var LPosMC:AWSprite;
	var previewColorLabel:JLabel;
	var previewColorIcon:PreviewColorIcon;
	var AAdjuster:JAdjuster;
	var RAdjuster:JAdjuster;
	var GAdjuster:JAdjuster;
	var BAdjuster:JAdjuster;
	var HAdjuster:JAdjuster;
	var SAdjuster:JAdjuster;
	var LAdjuster:JAdjuster;
	var hexText:JTextField;
    ...
    function createLabelToComponet(label:String, component:Component):Container{
    	var p:JPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
    	p.append(new JLabel(label));
    	p.append(component);

    	component.addEventListener(AWEvent.HIDDEN, function(e:Dynamic):Void{
    		p.setVisible(false);
    	});
    	component.addEventListener(AWEvent.SHOWN, function(e:Dynamic):Void{
    		p.setVisible(true);
    	});
    ...
    function getMixerPaneSize():IntDimension{
		var crm:Int = getColorRectMargin()*2;
	...
	function getMCsMarginSize():Int{
		return 4;
	}
	
	function getColorRectMargin():Int{
		return 1;
	}
    
    function getHSSize():IntDimension{
    	return new IntDimension(120, 100);
    }
    
    function getHS2LGap():Int{
    	return 8;
    }
    
    function getLSize():IntDimension{
    	return new IntDimension(40, 100);
    }
	
	function getLStripWidth():Int{
	...
	//------------------------------------------------------------------------
	function __AAdjusterValueChanged(e:Event):Void{
		if(!AAdjusterUpdating){
			updatePreviewColorWithHSL();
		}
	}
	function __AAdjusterValueAction(e:Event):Void{
		colorMixer.setSelectedColor(getColorFromMCViewsAndAAdjuster());
	}
	function __RGBAdjusterValueChanged(e:Event):Void{
	...
	function __RGBAdjusterValueAction(e:Event):Void{
		colorMixer.setSelectedColor(getColorFromRGBAAdjusters());
	}
	function __HLSAdjusterValueChanged(e:Event):Void{
	...
	function __HLSAdjusterValueAction(e:Event):Void{
	...
	function __HSMCPress(e:Event):Void{
		HSMC.addEventListener(MouseEvent.MOUSE_MOVE, __HSMCDragging);
		__HSMCDragging(null);
	}
	function __HSMCRelease(e:Event):Void{
	...
	function __HSMCDragging(e:Event):Void{
		countHSWithMousePosOnHSMCAndUpdateViews();
	}
	
	function __LMCPress(e:Event):Void{
		LMC.addEventListener(MouseEvent.MOUSE_MOVE, __LMCDragging);
		__LMCDragging(null);
	}
	function __LMCRelease(e:Event):Void{
		LMC.removeEventListener(MouseEvent.MOUSE_MOVE, __LMCDragging);		
		countLWithMousePosOnLMCAndUpdateViews();
		setSelectedColor(getColorFromMCViewsAndAAdjuster());
	}
	function __LMCDragging(e:Event):Void{
		countLWithMousePosOnLMCAndUpdateViews();
	}
	
	function countHSWithMousePosOnHSMCAndUpdateViews():Void{
	...
	function getHSWithPosOnHSMC(p:IntPoint):Dynamic{
		var hsSize:IntDimension = getHSSize();
		var minX:Int = getHSColorsStartX();
		var maxX:Int = minX + hsSize.width;
		var minY:Int = getHSColorsStartY();
		var maxY:Int = minY + hsSize.height;
	...
	function countLWithMousePosOnLMCAndUpdateViews():Void{
		var p:IntPoint = mixerPanel.getMousePosition();
	...
	function getLWithPosOnLMC(p:IntPoint):Float{
		var lSize:IntDimension = getLSize();
		var minY:Int = getLColorsStartY();
		var maxY:Int = minY + lSize.height;
		p.y = Std.int(Math.max(minY, Math.min(maxY, p.y)));
		return getLFromPos(p);
	}
	
	function getColorFromMCViewsAndAAdjuster():ASColor{
		return HLSA2ASColor(H_at_HSMC, L_at_LMC, S_at_HSMC, getAFromAAdjuster());
	}
	
	function updatePreviewColorWithHSL():Void{
		updatePreviewWithColor(getColorFromMCViewsAndAAdjuster());
	}
	
	function updatePreviewWithColor(color:ASColor):Void{
		previewColorIcon.setCurrentColor(color);
		previewColorLabel.repaint();
    	colorMixer.getModel().fireColorAdjusting(color);
	}
	
	function updateOthersWhenHSMCAdjusting():Void{
	...
	function updateOthersWhenLMCAdjusting():Void{
		var color:ASColor = getColorFromMCViewsAndAAdjuster();
		updateHexTextWithColor(color);
	...
	function getALabel():String{
		return "Alpha:";
	}
	function getRLabel():String{
		return "R:";
	}
	function getGLabel():String{
		return "G:";
	}
	function getBLabel():String{
		return "B:";
	}
	function getHLabel():String{
		return "H:";
	}
	function getSLabel():String{
		return "S:";
	}
	function getLLabel():String{
		return "L:";
	}
	function getHexLabel():String{
		return "#";
	}
	
	function updateMixerAllItemsWithColor(color:ASColor):Void{
		var hls:Dynamic = getHLS(color);
		var hh:Float = hls.h;
		var ss:Float = hls.s;
		var ll:Float = hls.l;
	...
	function updateHSLPosFromHLS(hh:Float, ll:Float, ss:Float):Void{
		var hsSize:IntDimension = getHSSize();
	...
	function updateRGBAdjustersWithColor(color:ASColor):Void{
	...
	function updateHLSAdjustersWithHLS(h:Float, l:Float, s:Float):Void{
		updateHAdjusterWithH(h);
		updateLAdjusterWithL(l);
		updateSAdjusterWithS(s);
	}
	
	function updateAlphaAdjusterWithColor(color:ASColor):Void{
		updateAAdjusterWithA(color.getAlpha());
	}

    var hexTextColor:ASColor;
    var hexTextUpdating:Bool;
    function updateHexTextWithColor(color:ASColor):Void{
    	if(!color.equals(hexTextColor)){
	    	...
	    	if(color == null){
	    		...
	    		hex = StringTools.hex(color.getRGB(), 6);
	    	}
            /*
	...
	function getHSColorsStartX():Int{
		return getMCsMarginSize() + getColorRectMargin();
	}
	function getHSColorsStartY():Int{
		return getMCsMarginSize() + getColorRectMargin();
	}
	function getLColorsStartY():Int{
		return getMCsMarginSize() + getColorRectMargin();
	}
	...
	function paintHSMC():Void{
		HSMC.graphics.clear();
		var g:Graphics2D = new Graphics2D(HSMC.graphics);
		var offset:Int = getMCsMarginSize();
		var thickness:Int = getColorRectMargin();
		var hsSize:IntDimension = getHSSize();
		var H:Int = hsSize.width;
		var S:Int = hsSize.height;
		
		var w:Int = H+thickness*2;
		var h:Int = S+thickness*2;
		...
		var colors:Array<Dynamic> = [0, 0x808080];
	...
	function paintLMCWithHS(hh:Float, ss:Float):Void{
		LMC.graphics.clear();
		var g:Graphics2D = new Graphics2D(LMC.graphics);
		var x:Int = getHSSize().width + getMCsMarginSize() + getColorRectMargin()*2 + getHS2LGap();
		var y:Int = getMCsMarginSize();
		
		var thickness:Int = getColorRectMargin();
		var lSize:IntDimension = getLSize();
		var w:Int = getLStripWidth() + thickness*2;
		var h:Int = lSize.height + thickness*2;
		...
		var colors:Array<Dynamic> = [0xFFFFFF, HLSA2ASColor(hh, 0.5, ss, 100).getRGB(), 0];
	...
	function paintLMCWithColor(color:ASColor):Void{
		var hls:Dynamic = getHLS(color);
		var hh:Int = hls.h;
		var ss:Int = hls.s;
		paintLMCWithHS(hh, ss);
	}
	function paintLMC():Void{
		paintLMCWithColor(getSelectedColor());
	}
	function paintLPosMC():Void{
	...
	function createMixerPanel():JPanel{
	...
	function createPreviewColorLabel():JLabel{
		...
		var margin:Int = getMCsMarginSize();
	...
	function createHexTextField():JTextField{
		return new JTextField("000000", 6);
	}
	
	function createAdjusters():Void{
	...
	function createAAdjuster():JAdjuster{
	...
	function createRAdjuster():JAdjuster{
		var adjuster:JAdjuster = new JAdjuster(4, JAdjuster.VERTICAL);
		adjuster.setValues(255, 0, 0, 255);
		return adjuster;
	}
	function createGAdjuster():JAdjuster{
		var adjuster:JAdjuster = new JAdjuster(4, JAdjuster.VERTICAL);
		adjuster.setValues(255, 0, 0, 255);
		return adjuster;
	}
	function createBAdjuster():JAdjuster{
		var adjuster:JAdjuster = new JAdjuster(4, JAdjuster.VERTICAL);
		adjuster.setValues(255, 0, 0, 255);
		return adjuster;
	}
	function createHAdjuster():JAdjuster{
		var adjuster:JAdjuster = new JAdjuster(4, JAdjuster.VERTICAL);
	...
	function createSAdjuster():JAdjuster{
		var adjuster:JAdjuster = new JAdjuster(4, JAdjuster.VERTICAL);
	...
	function createLAdjuster():JAdjuster{
		var adjuster:JAdjuster = new JAdjuster(4, JAdjuster.VERTICAL);
		adjuster.setValues(0, 0, 0, 100);
		adjuster.setValueTranslator(
			function(value:Float):String{
	...
	static function getHLS(color:ASColor):Dynamic{
	 ...
	 */
	static function HLSA2ASColor(H:Float, L:Float, S:Float, alpha:Float):ASColor{
	...
	static function rgb2Hls(rr:Float, gg:Float, bb:Float):Dynamic{
