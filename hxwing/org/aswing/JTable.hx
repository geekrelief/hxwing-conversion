 ...
 * @author iiley
 */
class JTable extends Container, implements org.aswing.Viewportable, implements org.aswing.event.TableModelListener, implements org.aswing.table.TableColumnModelListener, implements org.aswing.event.CellEditorListener, implements org.aswing.LayoutManager {
		
	public static var INT_MAX:Int = (2<<30) -1;	
 	public static var INT_MIN:Int = 2<<30;
 	/**
 	 * The default unit/block increment, it means auto count a value.
 	 */
 	public static var AUTO_INCREMENT:Int = INT_MIN;
	...
	var cellPane:Container;
	var headerPane:Container;
	var tableHeader:JTableHeader;
	var rowCells:Array<Dynamic>;
	var rowHeight:Int;
	var rowMargin:Int;
	...
	var autoCreateColumnsFromModel:Bool;
	var preferredViewportSize:IntDimension;
	var rowSelectionAllowed:Bool;
	var cellSelectionEnabled:Bool;
	var cellEditor:TableCellEditor;
	var editingColumn:Int;
	var editingRow:Int;
	var defaultRenderersByColumnClass:HashMap;
	var defaultEditorsByColumnClass:HashMap;
	var selectionForeground:ASColor;
	var selectionBackground:ASColor;
	//private var surrendersFocusOnKeystroke:Boolean;
	var columnSelectionAdjusting:Bool;
	var rowSelectionAdjusting:Bool;
	 ...
	 */	
	function initWithModels(?dm:TableModel=null, ?cm:TableColumnModel=null, ?sm:ListSelectionModel=null):Void{
		setLayout(this);
	...
	public override function setUI(newUI:ComponentUI):Void{
		if (Std.is( newUI, TableUI)){
			...
			throw ("JTable ui should implemented TableUI interface!");
		}
	}
	...
	function updateSubComponentUI(componentShell:Dynamic):Void{
	...
	public function setRowHeight(rowHeight:Int):Void{
		if (rowHeight < 1){
			...
			throw ("New row height less than 1");
		}
	...
	function boundColumn(col:Int):Int{
	...
	public function getSelectedRows():Array<Dynamic>{
		...
		var rvTmp:Array<Dynamic> = new Array();
		var i:Int = iMin;
		while (i <= iMax){
	...
	function changeSelectionModel(sm:ListSelectionModel, index:Int, 
	...
	function getViewStartPoint():IntPoint{
	...
	function viewIndexForColumn(aColumn:TableColumn):Int{
		var cm:TableColumnModel = getColumnModel();
		for (column in 0...cm.getColumnCount())
		{
	...
	function setWidthsFromPreferredWidths(inverse:Bool):Void{
		var insets:Insets = getInsets();
	...
	function accommodateDelta(resizingColumnIndex:Int, delta:Int):Void{
	...
	function adjustSizes3(target:Int, r:Resizable3, inverse:Bool):Void{
	...
	function adjustSizes2(target:Int, r:Resizable2, limitToRange:Bool):Void{
	...
	function tableRowsInserted(e:TableModelEvent):Void{
	...
	function tableRowsDeleted(e:TableModelEvent):Void{
		var start:Int = e.getFirstRow();
		var end:Int = e.getLastRow();
		if (start < 0){
			start = 0;
	...
	function initializeLocalVars():Void{
...
//		editorRemover = null;
	...
	function getLastTotalColumnWidth():Int{
	...
	function layoutCell(row:Int, column:Int, cellRect:IntRectangle, cr:Int, cc:Int):Void{
...
//        hasFocus = (rowIsLead && colIsLead) && isFocusOwner();
	...
	function synCreateCellInstances():Void{
		...
		if(rowCells.length < needNum){
			...
			for(i in 0...addNum){
				var columnCells:Array<Dynamic> = new Array();
	...
	function removeCells(removed:Array<Dynamic>):Void{
	...
	function addCellToContainer(cell:TableCell):Void{
	...
	function removeCellFromeContainer(cell:TableCell):Void{
	...
	function fireStateChanged(?programmatic:Bool=true):Void{
	...
	function restrictionViewPos(p:IntPoint):IntPoint{
	...
	function getViewMaxPos():IntPoint{
	 ...
	 * @see #addSelectionListener()
	 */	
	public function addColumnSelectionListener(listener:Dynamic, ?priority:Int=0, ?useWeakReference:Bool=false):Void{
	 ...
	 */	
	public function removeColumnSelectionListener(listener:Dynamic):Void{
