...
class PropertyTableModel extends AbstractTableModel, implements ListDataListener {
	 ...
	 */
	public override function getValueAt(rowIndex:Int, columnIndex:Int):Dynamic{
		...
		if(translator != null){
			if(Std.is( translator, PropertyTranslator)){
				...
				throw ("Translator must be a PropertyTranslator or a Function : " + translator);
			}
		}else{
			if(key == "."){
				return info;
	...
	public override function setValueAt(aValue:Dynamic, rowIndex:Int, columnIndex:Int):Void{
		var info:Dynamic = list.getElementAt(rowIndex);
		var key:String = properties[columnIndex];
