...
class TableSorter extends AbstractTableModel, implements TableModelListener {
    ...
    static var EMPTY_DIRECTIVE:Directive;
    public static var NUMBER_COMAPRATOR:Dynamic;
    public static var LEXICAL_COMPARATOR:Dynamic;
    
    static var inited:Bool = false;

    var tableModel:TableModel;
    var viewToModel:Array<Dynamic>; //Row[]
    var modelToView:Array<Dynamic>; //int[]
    var columnSortables:Array<Dynamic>;

    var tableHeader:JTableHeader;
    var tableModelListener:TableModelListener;
    var columnComparators:HashMap;
    var sortingColumns:Array<Dynamic>;
    ...
    function clearSortingState():Void {
        viewToModel = null;
        modelToView = null;
    }
    ...
    function getDirective(column:Int):Directive {
    ...
    function sortingStatusChanged():Void {
        clearSortingState();
	 ...
	 */
    public function setColumnComparator(columnClass:String, comparator:Dynamic):Void {
        if (comparator == null) {
	 ...
	 */
    public function getComparator(column:Int):Dynamic {
        var columnType:String = tableModel.getColumnClass(column);
        var comparator:Function = columnComparators.getcast( (columnType), Function);
        if (comparator != null) {
            return comparator;
        }
    ...
    function getViewToModel():Array<Dynamic> {
        if (viewToModel == null) {
    ...
    function sortImp(row1:Row, row2:Row):Int{
    ...
    function getModelToView():Array<Dynamic> { //int[]
        if (modelToView == null) {
    ...
    public override function getColumnClass(column:Int):String {
        return tableModel.getColumnClass(column);
    }

    public override function isCellEditable(row:Int, column:Int):Bool {
        return tableModel.isCellEditable(modelIndex(row), column);
    }

    public override function getValueAt(row:Int, column:Int):Dynamic {
        return tableModel.getValueAt(modelIndex(row), column);
    }

    public override function setValueAt(aValue:Dynamic, row:Int, column:Int):Void {
    ...
    var pressedPoint:IntPoint;
    function __mousePress(e:MouseEvent):Void {
    	var header:JTableHeader = cast( e.currentTarget, JTableHeader);
    	pressedPoint = header.getMousePosition();
    }

    function __mouseRelease(e:ReleaseEvent):Void {
    ...
    function nextSortingStatus(curStatus:Int, shiftKey:Bool):Int{
